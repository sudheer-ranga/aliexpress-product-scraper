...
const getFeedbackData = (feedbackHtml) => {
  ...
  $(".feedback-list-wrap .feedback-item").each((index, element) => {
    const $elm = $(element);
    let name = $elm.find(".user-name").text().trim();

    let country = $elm.find(".user-country").text().trim();
    ...
    $elm.find(".user-order-info > span").each((index, infoKey) => {
      const key = $(infoKey).find("strong").text().trim();

      $(infoKey).find("strong").remove();

      info[key] = $(infoKey).text().trim();
    });
    ...
    $elm.find(".r-photo-list > ul > li").each((index, photo) => {
      const url = $(photo).find("img").attr("src");
      photos.push(url);
    });
